...
export type FlowKind = "define" | "compare" | "build" | "calculate" | "classify" | "sequence" | "cause" | "judge";
...
type ModuleSeed = { code: string; title: string; blurb: string; lab?: string; lessons: string[] };
...
const authored: Record<string, Authored> = { ...foundationsContent, ...appliedContent, ...professionalContent };
...
const flowMinutes: Record<FlowKind, number> = { define: 3, compare: 4, build: 5, calculate: 5, classify: 4, sequence: 4, cause: 4, judge: 5 };
...
function makeLesson(seed: ModuleSeed, title: string, order: number, levelCode: "FND" | "APP" | "PRO", levelTitle: string): Lesson {
  ...
  return {
    ...
    masteryTags: masteryFor(title, seed.title), status: "Draft", xp: 10,
...
const makeModules = (seeds: ModuleSeed[], levelCode: "FND" | "APP" | "PRO", levelTitle: string): CourseModule[] => seeds.map((seed) => ({
  code: seed.code, title: seed.title, lab: seed.lab, description: seed.blurb,
...
export const flowKinds: FlowKind[] = ["define", "compare", "build", "calculate", "classify", "sequence", "cause", "judge"];
